...
function testa_comunicacao() {
    ...
    if ($('#bd_id').val() == "") {
        ...
        $.post('testar_comunicacao.php', { bd_id: $('#bd_id').val() }).done(function (data) {
...
function executa_backup() {
    ...
    if ($('#bd_id').val() == "") {
        ...
        $.post('executa_backup.php', { bd_id: $('#bd_id').val() }).done(function (data) {
...
function restaurar_backup() {
    ...
    if ($('#bd_id').val() == "") {
        ...
        $.post('restaura_backup.php', { bd_id: $('#bd_id').val(), documento_id: $('#documento_id').val() }).done(function (data) {
...
function reduzir_backup() {
    ...
    if ($('#bd_id').val() == "") {
        ...
        $('.gif').prop('hidden', false);
        $.post('reduz_backup.php', { bd_id: $('#bd_id').val() }).done(function (data) {
...
function transferencia_por_app() {

    $('#transferencia_usuario').hide();
    $('#transferencia_app').show();
    $('#cancelar').prop('disabled', false);
    $('#transferir').prop('disabled', false);
...
function cancelar_transferencia_arquivos() {
    ...
    $('#transferencia_arquivos_app').prop('checked', false);
    $('#cancelar').prop('disabled', true);
    $('#transferir').prop('disabled', true);
    $('#bd_id').val("");
    $('#bd_id').selectpicker('refresh');
    $('#servidor_id').val("");
...
function retorna_usuario_selecionado() {
    ...
    $.post('retorna_servidor_atual.php', { bd_id: $('#bd_id').val() }).done(function (data) {
...
function transfere_arquivo() {
    ...
    if ($('#bd_id').val() == "") {
        ...
        $.post('retorna_info_transferencia.php', { bd_id: $('#bd_id').val(), servidor_id: $('#servidor_id').val() }).done(function (data) {
            ...
            $.confirm({
                ...
                confirm: function () {
                    ...
                    $('#gif').show();
                    $.post('transfere_arquivos.php', { bd_id: $('#bd_id').val(), servidor_id: $('#servidor_id').val() }).done(function (data) {
...
function verifica_plat_servidor_bkp() {
    ...
    if ($('#servidor_plataforma').val() == "Linux") {
        ...
        $('#inst_usuariopass').show(250);

    } else {

        $('#inst_usuariopass').hide();
    }
